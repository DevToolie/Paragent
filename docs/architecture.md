...
| 5 | ~~**The cache has a write path only.**~~ **Fixed by [#63](https://github.com/DevToolie/Paragent/issues/63)** — `CacheStore` gained `get`/`list`, and `JsonlCacheStore` persists pool and tenant rows to two append-only files | `src/cache/store.ts` | **Still no cache *hit*.** Nothing reads from the store to skip fresh reasoning, so there is no hit-rate yet; and confidence/self-invalidation remain unimplemented | [#64](https://github.com/DevToolie/Paragent/issues/64) |
