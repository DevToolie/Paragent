...
| 8 | **Cache hit-rate is missing from §9.** `buildGateReport` returns seven sections; hit-rate is not one of them. Since #118 the *inputs* exist — `program_source` on step and run rows — so the blocker is now the aggregate section, not the mechanism | `src/metrics/aggregate.ts:282-291` | One §9 secondary metric is still not reported | [#67](https://github.com/DevToolie/Paragent/issues/67) |
