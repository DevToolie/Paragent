...
| 5 | ~~**The cache has a write path only.**~~ Store landed in [#63](https://github.com/DevToolie/Paragent/issues/63); confidence, self-invalidation and the repair rewrite in [#64](https://github.com/DevToolie/Paragent/issues/64) ([ADR-0009](./decisions/ADR-0009-cache-confidence.md)); the program entity and a fail-closed resolver in [#120](https://github.com/DevToolie/Paragent/issues/120) ([ADR-0013](./decisions/ADR-0013-cache-program-entity.md)) | `src/cache/store.ts`, `src/cache/confidence.ts`, `src/cache/resolve.ts` | **Still no cache *hit*.** `resolveProgram` can now turn rows into a whole program — or refuse — but it has **no caller in `src/` outside its own tests**, so nothing reads the cache to skip fresh reasoning and there is still no hit-rate and no amortization behind it. Confidence is recorded during Track 1 and deliberately never acted on, including on the read path | [#118](https://github.com/DevToolie/Paragent/issues/118) |
